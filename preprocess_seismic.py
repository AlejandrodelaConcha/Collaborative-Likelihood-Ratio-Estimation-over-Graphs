# -----------------------------------------------------------------------------------------------------------------
# Title:  preprocess_seismic
# Author(s):  
# Initial version:  2024-01-15
# Last modified:    2025-02-26             
# This version:     2025-02-26
# -----------------------------------------------------------------------------------------------------------------
# Objective(s): The goal of this script is to recover the graph structure and the waveforms associated 
#               with a given event coming from a specified ID.
# -----------------------------------------------------------------------------------------------------------------
...
# epicenter: the location of the epicenter.
    ...
    inventory = client.get_stations(latitude=cat[0].origins[0].latitude,
                                ...
                                channel="HHZ",
                                level="channel",
                                starttime = otime-seconds_before-60-10,
                                endtime = otime-seconds_before+10)
    station=network[0]
            ...
            for i in range(3):
                try:
                    aux_waveform=client.get_waveforms(code, station, "*",sensor_name[i],
                          otime-seconds_before-60-10, otime-seconds_before+10,attach_response=True)
...
# event_id: identifier of the event, available on the GEONET webpage.
    ...
    inventory = client.get_stations(latitude=cat[0].origins[0].latitude,
                                    ...
                                    channel="HHZ",
                                    level="channel",
                                    starttime = otime-seconds_before-60,
                                    endtime = otime-seconds_before)
 
    if(int(event_id[:4])>=2024):
        network=[station.code for station in inventory[1].stations]
    else:
        network=[station.code for station in inventory[0].stations]
    waveforms,epicenter=get_data(network,event_id,seconds_before)   
    coordinates={}
...
### Events considered on the paper 
#event_id="2023p741652"  
#event_id="2021p405872"
